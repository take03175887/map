        ...
        <body class="bg-gray-100">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <form action="/" method="GET">
                    ...
                    <button type="submit" class="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded">検索</button>
                </form>
                    ...
                    @foreach ($cards as $card)
                        <div class="mt-4 bg-white rounded-md overflow-hidden shadow-md">
                            ...
                            <div class="p-4">
                                ...
                                <form action="/card/{{ $card->getId() }}" id="form_{{ $card->getId() }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="deletePost({{ $card->getId() }})" class="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded">削除</button>
                                    <script>
